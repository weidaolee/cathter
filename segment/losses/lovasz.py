...
class LovaszLoss(BaseLossDecorator):
    def __init__(
        self,
        base_loss=BaseLoss(),
        classes=[1],
        ...
        super(LovaszLoss, self).__init__(base_loss=base_loss, factor=factor)
        self.classes = classes
        self.per_image = per_image
        self.ignore = ignore

        self.name = "LovaszLoss"
    ...
    def _forward(self, output, target):
        ...
        if self.per_image:
            loss = mean(
                ...
                ) for prob, targ in zip(output, target))
        else:
